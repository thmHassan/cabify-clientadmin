...
const DriverDetails = () => {
    ...
    const handleSaveBank = async () => {
        ...
        try {
            const formDataObj = new FormData();
            formDataObj.append("id", driverId);
            // Required driver fields (API validates these)
            formDataObj.append("name", formData.name || "");
            formDataObj.append("email", formData.email || "");
            formDataObj.append("country_code", formData.country_code || "");
            formDataObj.append("phone_no", formData.phone_no || "");
            ...
            formDataObj.append("bank_account_number", formData.bank_account_number || "");
            formDataObj.append("account_holder_name", formData.account_holder_name || "");
            formDataObj.append("bank_phone_no", formData.bank_phone_no || "");
            formDataObj.append("iban_no", formData.iban_no || "");
            const response = await apiEditDriverManagement(formDataObj);
            if (response?.data?.success === 1 || response?.status === 200) {
            ...
            toast.error(
                error?.response?.data?.message || error?.message || "Error saving bank information"
            );
        } finally {
