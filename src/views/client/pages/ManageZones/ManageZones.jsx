...
const ManageZones = () => {
  ...
  // editingCell tracks which cell is being edited: { rowIndex, colIndex }
  ...
  // Returns available backup options for a given row & colIndex,
  // excluding currently selected backups EXCEPT the one at this colIndex (so you can re-pick it).
  const filteredBackupOptions = (rowIndex, colIndex = null) => {
    const row = rows[rowIndex];
    return plots.filter((p) => {
      if (p.id === row.selectedPlot?.id) return false;
      // Exclude already-selected backups, but allow the one at colIndex to show for re-selection
      return !row.selectedBackups.some((id, idx) => {
        if (colIndex !== null && idx === colIndex) return false; // allow current slot's value
        return id === p.id;
      });
    });
  };
              ...
              >
                ...
                {Array.from({ length: Math.max(plots.length - 1, 0) }).map((_, colIndex) => (
                  <td key={colIndex} className="border-l px-2 py-2">
                    {!row.selectedPlot ? (
                    ...
                    ) : row.selectedBackups[colIndex] && !isEditing(rowIndex, colIndex) ? (
                      // ✅ Already selected — show name with edit (pencil) button
                      <div className="flex items-center gap-1 group">
                        <span className="font-semibold">
                          {plots.find((p) => p.id === row.selectedBackups[colIndex])?.name}
                        </span>
                        <button
                          title="Change backup plot"
                          onClick={() => setEditingCell({ rowIndex, colIndex })}
                          className="ml-1 text-gray-400 hover:text-blue-500 transition-colors opacity-0 group-hover:opacity-100"
                        >
                          {/* Pencil icon */}
                      ...
                      // ✅ Editing mode — show dropdown with cancel option
                      <div className="flex items-center gap-1">
                        <select
                          className="border p-1 rounded text-sm w-full"
                          defaultValue={row.selectedBackups[colIndex] || ""}
                          onChange={(e) => handleSelectBackup(e.target.value, rowIndex, colIndex)}
                        >
                          <option value="">Select Backup Plot</option>
                          {filteredBackupOptions(rowIndex, colIndex).map((fp) => (
