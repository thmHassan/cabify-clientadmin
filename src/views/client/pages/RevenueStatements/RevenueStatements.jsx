...
const RevenueStatements = () => {
  ...
  useEffect(() => {
    const fetchFinancialSummary = async () => {
      ...
      try {
        ...
        const response = await apiGetRevenueCard(params);

        if (response?.data?.success === 1) {
          setFinancialSummary({
            ridesEarning: response.data.ridesEarning || 0,
            jobOfferedCount: response.data.jobOfferedCount || 0,
            jobAcceptedCount: response.data.jobAcceptedCount || 0
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchRideHistory = async () => {
      ...
      try {
        ...
        const response = await apiGetRevenueHistory(params);

        if (response?.data?.success === 1) {
          const ridesData = response.data.rides;
          setRideHistory(ridesData.data || []);
          setTotalPages(ridesData.last_page || 1);
        }
      } catch (error) {
  ...
  const DASHBOARD_CARDS = [
    {
      title: "Ride Earnings",
      value: financialSummary.ridesEarning.toString(),
    ...
    {
      title: 'Jobs Offered',
      value: financialSummary.jobOfferedCount.toString(),
    ...
    {
      title: 'Jobs Accepted',
      value: financialSummary.jobAcceptedCount.toString(),
            ...
            >
              ...
              >
                <svg width="18" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M1 1L10 10L19 1" stroke="#333333" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </span>
            </div>

            {isOpen && (
              <ul className="absolute z-10 mt-1 w-full bg-white border-[1px] border-[#6C6C6C] rounded-[6px] shadow-lg max-h-60 overflow-y-auto">
